...
df_generated_input = pd.read_json( params['output_results'] + '1_generation_[max:200]_01.json' )
...
# %%
#tst
def tst_run_code_rational_sampling_set(exp):
    gc.collect()
    torch.cuda.empty_cache()
    ...
    test_arr_rationals.to_json( params['output_results'] + 'rationales_[t_1000]_[max_src_200]_[max_tgt_90]_01_' + EXP )
    return test_arr_rationals
#df_test_run = tst_run_code_rational()

# %%
def tst_run_code_rational_all_set(exp):
    gc.collect()
    torch.cuda.empty_cache()
    tensor_n = 1000 #df_generated_input.shape[0]
    EXP = exp
...
for i in df_generated_input.columns[4:-1]: #Only Generated Sequences 
    df_test_run = tst_run_code_rational_all_set(i)
